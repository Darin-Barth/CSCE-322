...
manyPlayersManyMoves ma [] = ma
manyPlayersManyMoves ma mos = manyPlayersManyMovesHelper ma maxPlayers startingPlayer mos
	where 
	maxPlayers = numPlayers ma
	startingPlayer = 1
	
manyPlayersManyMovesHelper :: [[Char]] -> Int -> Int -> [Char] -> [[Char]]
manyPlayersManyMovesHelper maze maxPlayers currentPlayer [] = maze
manyPlayersManyMovesHelper maze maxPlayers currentPlayer moves 
		| mod currentPlayer maxPlayers == 0 =  manyPlayersManyMovesHelper maxMaze maxPlayers newPlayer t 
		| otherwise = manyPlayersManyMovesHelper normalMaze maxPlayers newPlayer t 
			where 
			maxMaze = onePlayerHelper maze h lastPlayer
			normalMaze = onePlayerHelper maze h currentPlayerChar
			lastPlayer=chr (48+maxPlayers)
			newPlayer = currentPlayer+1
			currentPlayerChar = chr (48+adjustedPlayer)
			adjustedPlayer = mod currentPlayer maxPlayers
			(h:t) = moves
			

numPlayers :: [[Char]] -> Int
numPlayers maze = numPlayersHelper maze 49 0


numPlayersHelper :: [[Char]] -> Int -> Int -> Int
numPlayersHelper maze currPlay count 
	| find2D player maze == [] = count
	| otherwise = numPlayersHelper maze 49 newCount
		where 
		newCount = count+1
		player = chr (currPlay + count)
...
onePlayerHelper maze nextDirec player 
	...
	-- hence, buffMaze is an intermediate in these swaps. Might need to include previous direction as a function argument for 2nd player search
		...
		newCoord 		= coordinatesOfPath nextDirec prevCoord --return tuple here. 
